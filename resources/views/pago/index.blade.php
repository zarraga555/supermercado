      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <h2><u><a href="#" class="btn btn-primary" class="btn btn-primary" data-toggle="modal"
                data-target="#CreateModal">Crear Nuevo Metodo Pago</a></u></h2>
        </ol>
      </div>
    </div>
      <!--MESSAGES-->
...
<div class="panel-body">
  <div id="list-Pago"></div>
</div>

@include('pago.modalCreate')
@include('pago.modalEdit')
@include('pago.modalDelete')
<script>
  //-----------------------------------------------------
  //----------------------PAGINATION---------------------
  //-----------------------------------------------------
  ...
  var listPago = function () {
    $.ajax({
      type: 'get',
      url: '{{ url('listPago') }}',
      success: function (data) {
        $('#list-Pago').empty().html(data);
      }
    });
  }
    ...
    //---------------------EDIT----------------------------
    //-----------------------------------------------------
    var Mostrar = function (pago) {
        var route = "{{ url('pago') }}/" + pago + "/edit";
    ...
    $('#enviar').click(function () {
        ...
        var token = $("input[name=_token]").val();
        var route = "{{ route('metodoPago.store') }}"
        $.ajax({
            url: route,
            headers: {
                'X-CSRF-TOKEN': token
    ...
    $('#delete').click(function () {
        var pago = $("#idDelete").val();
        var token = $("input[name=_token]").val();
        var route = "{{ URL('pago') }}/" + pago;
        $.ajax({
            url: route,
            headers: {
                'X-CSRF-TOKEN': token
